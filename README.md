...
(`candidata` / `aprovada` / `rejeitada`) you flip whenever you actually
decide.
...
modal is sticky — nothing ever silently reclassifies it back.
...
kind, purpose, your personal note, plus save/delete/refresh — matching My
...
`~/.claude/.skill-library` — your data survives a reinstall.
...
capability set, `RestrictAddressFamilies=AF_UNIX` (it cannot open a network
socket beyond that one exception's HTTPS call — the unit still needs
network access for `refresh_repo` to work; if you don't need that button,
tighten the unit further), a syscall filter that excludes the
...
sticky, survives a per-item refresh.
