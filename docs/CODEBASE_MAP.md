---
last_mapped: 2026-09-02T17:28:10Z
total_files: 29
total_tokens: 63324
---
...
> Auto-generated by Cartographer. Last mapped: 2026-09-02T17:28:10Z
...
└── src/
    ...
    ├── detect_installed.py           one-time Claude/Codex installed-detection (never in the live path)
...
tests/
    ...
    ├── test_detect_installed.py      scan_claude/codex_installed, mark_installed, main() round-trip
...
- `appendInstalledBadges(container, item)` appends a "Claude" and/or "Codex" `<span class="badge badge-installed-claude|codex">` to `container` when `item.installed_claude`/`installed_codex` is truthy (set by the offline `detect_installed.py`, absent — falsy — on anything that hasn't been through it). Called from both `renderItem()` (into `.card-badges`, after the purpose badge) and `renderModal()` (into a `.modal-title-row` wrapping the `<h2>`, so the badges sit right next to the item name instead of fighting the header's `justify-content: space-between` with the close button).
- `renderItem(item)`'s card is 3 stacked lines, not one flex row: `.card-badges` (status/kind/purpose/installed, wraps freely), then `.card-name` (its own block — a long name used to squeeze the badges into a cramped, letter-wrapped column when they shared one row), then `.card-desc`, then `.card-meta` (stars, a small inline `GITHUB_MARK_SVG` icon, the repo link).
- `fieldLabelText(field)` prefixes `dev_note`'s label with "✉️🏠" (envelope+house) in both `renderModal()`'s field loop and the add-item modal's — a visual cue that it's the *internal* note, as opposed to `personal_note` (the user's own, plain-labeled).
...
- ~~`docs/nginx-snippet.conf` had drifted from what `install.sh` actually injects~~ — **fixed**: it set a stale `Content-Type: application/json` proxy header and was missing `proxy_read_timeout`; now matches `install.sh`'s real injected block (`proxy_set_header Host $host; proxy_read_timeout 30s;`) and `docker/nginx.conf`.
