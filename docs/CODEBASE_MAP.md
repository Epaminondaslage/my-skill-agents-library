---
last_mapped: 2026-09-02T01:22:24Z
total_files: 25
total_tokens: 43366
---
...
> Auto-generated by Cartographer. Last mapped: 2026-09-02T01:22:24Z
...
- State: `items`, `filterStatus`, `filterKind`, `filterPurpose`, `sortBy`, `openModalId`, plus search state `searchQuery`, `ghResults`, `ghLoading`, `ghError`, `ghAddedRepos` — plain globals, re-rendered wholesale on every mutation via `render()`.
...
- `renderSearchBox()` builds the local search input (filters `items` by name/repo/function substring, combined with the status/kind/purpose filters, purely client-side) plus a GitHub-mark button and a "+ incluir no catálogo" shortcut that scrolls to and prefills the add form.
