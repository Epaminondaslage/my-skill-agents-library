...
- `renderSections()` groups filtered items by `purpose` in fixed order, one colored section per purpose — including empty ones, which render the header bar (with a "(0)" count) plus a "nenhum item aqui" placeholder instead of being omitted, so the tab bar's purpose colors stay a stable landmark whether or not a purpose currently has matches.
