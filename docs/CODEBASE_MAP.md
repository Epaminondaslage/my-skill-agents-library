...
- `renderSections()` groups filtered items by `purpose` in fixed order, one colored section per purpose — including empty ones, which still render the header bar (just the purpose label, no count — the count already lives on the tab above) plus a "nenhum item aqui" placeholder instead of being omitted, so the tab bar's purpose colors stay a stable landmark whether or not a purpose currently has matches.
