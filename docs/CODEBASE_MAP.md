...
- State: `items`, `filterStatus`, `filterKind`, `filterPurpose`, `sortBy`, `openModalId`, plus search state `searchQuery`, `ghResults`, `ghLoading`, `ghError`, `ghAddedRepos`, plus i18n state `lang` — plain globals, re-rendered wholesale on every mutation via `render()`.
